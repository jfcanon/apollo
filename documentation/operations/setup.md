...
The `/console` page (`https://apollo.ygdcbtmc4u.workers.dev/console?token=<DASHBOARD_SHARED_SECRET>`) works on macOS Chrome and Safari with headphones for hands-free conversations.

### Prerequisites
- Mac with macOS 13+ (for `enumerateDevices` support)
...
### First Run
1. Open `https://apollo.ygdcbtmc4u.workers.dev/console?token=<apollo-dashboard-secret>` in Chrome or Safari
2. Allow microphone permission when prompted
3. Enter the device secret (`apollo-device-secret`) when prompted
4. Select your headset as **Input** device in the dropdown (preferences persist in `localStorage`)
5. **Output device selection works in Chrome only** (Safari lacks `AudioContext.setSinkId`); on Safari, choose output in macOS System Settings → Sound
6. Check **Headphones (barge-in while speaking)** to enable interrupting Jarvis
7. Click **Connect** — status shows "connected", orb idle

### Controls
| Action | Mouse | Keyboard |
|--------|-------|----------|
| Push-to-talk | Click **Talk** to start, click again to end | Hold **Space** (sends `wake`/`audio_end`) |
...
**Note:** The Talk button is click-to-toggle (not hold). The Space key is true push-to-hold.

### Visual States
The orb reflects `ui_state` from the server (status shows transport state only):
...
### Protocol Notes
- `hello.deviceId = "mac-console"` — sent on connect for future telemetry/logging distinction from ESP32 (`esp32-...`) and phone (`phone-console`)
...
| No audio output (Chrome) | Re-select output in page dropdown; check macOS System Settings → Sound → Output |
| No audio output (Safari) | Choose output in macOS System Settings → Sound (page dropdown hidden) |
